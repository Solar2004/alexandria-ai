...
Regla: al terminar cualquier unidad → PLAN → registra TASK (alx task add) + consulta MEMORIA (mcp memory.recall) → verifica SKILLS necesarias (alx skills-fetch --search si falta) → activa PLUGINS que apliquen (/caveman, /ecc) → DIVIDE en subagentes si es grande (Task) → si detectas patrón recurrente crea HARNESS por proyecto (alx harness-new, vive en .alexandria/) → usa MCP (phalanx.status, cost_report) si necesitas estado → VERIFICA (tests/evidencia real) → CRITICA → MEJORA. Un solo pase sin verificar = INCOMPLETO.
